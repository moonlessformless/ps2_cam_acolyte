...
#define MAX_BATCH_REPLY_COUNT 50000
    ...
    auto InitSocket() -> void {
        ...
        // BEGIN PS2 CAM ACOLYTE MOD - use a 16ms timeout when connecting to avoid stalls when PCSX2 isn't running
        // Set the socket to non-blocking mode
        sock_state = false; // this will be set to true if successful

        u_long mode = 1;
        ioctlsocket(sock, FIONBIO, &mode);

        int timeout = 16; // timeout in milliseconds
        ...
        if (result == SOCKET_ERROR) {
            if (WSAGetLastError() == WSAEWOULDBLOCK) {
                ...
                timeout.tv_sec = 0;
                timeout.tv_usec = 16000; // 16 milliseconds
  ...
  public:
    ...
    struct BatchCommand {
         ...
         */
        // BEGIN PS2Campanion mod - RVO means the temporaries returned will destruct in some cases
        //~BatchCommand() {
        void Free() {
        // END PS2Campanion mod
  ...
  public:
    ...
    // BEGIN PS2 CAM ACOLYTE MOD - allowing a return buffer to avoid repeated allocations for string calls
    auto GetReply(const Y &cmd, int place, std::vector<char>* optional_return_buffer = nullptr) {
    // END PS2 CAM ACOLYTE MOD
        ...
        if constexpr (T == MsgRead8)
            ...
            // BEGIN PS2 CAM ACOLYTE MOD - allowing a return buffer to avoid
            // repeated allocations for string calls
            if (optional_return_buffer != nullptr)
            {
                optional_return_buffer->resize(size);
                memcpy(optional_return_buffer->data(), &buf[loc + 4], size);
                return optional_return_buffer->data();
            }
            else
            {
